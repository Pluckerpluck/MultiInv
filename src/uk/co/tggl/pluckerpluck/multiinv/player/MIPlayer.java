...
public class MIPlayer {
    ...
    public void loadInventory(String group, String inventoryName) {
        ...
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            MIInventory inventory = MIYamlFiles.con.getInventory(
                    player.getName(), group, inventoryName);
    ...
    public void saveInventory(String group, String inventoryName) {
        ...
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            ...
            MIYamlFiles.con.saveInventory(player.getName(), group, inventory,
                    inventoryName);
        } else {
            MIPlayerFile config = new MIPlayerFile(player, group);
    ...
    public void loadEnderchestInventory(String group, String inventoryName) {
        ...
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            MIEnderchestInventory inventory = MIYamlFiles.con
                    .getEnderchestInventory(player.getName(), group,
                            inventoryName);
            inventory.loadIntoInventory(enderchest);
        } else {
            MIPlayerFile config = new MIPlayerFile(player, group);
            MIEnderchestInventory inventory = config
                    .getEnderchestInventory(inventoryName);
            inventory.loadIntoInventory(enderchest);
        }
    }
    ...
    public void saveEnderchestInventory(String group, String inventoryName) {
        ...
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            MIYamlFiles.con.refreshConnection();
            MIYamlFiles.con.saveEnderchestInventory(player.getName(), group,
    ...
    public void loadHealth(String group) {
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            ...
            MIPlayerFile config = new MIPlayerFile(player, group);
            double health = config.getHealth();
            if(health < 0) {
                health = 0;
            }
    ...
    public void saveHealth(String group) {
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            if(player.getHealth() < 0) {
                ...
                MIYamlFiles.con.saveHealth(player.getName(), group,
                        player.getHealth());
            }
        } else {
            MIPlayerFile config = new MIPlayerFile(player, group);
    ...
    public void loadGameMode(String group) {
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            MIYamlFiles.con.refreshConnection();
            GameMode gameMode = MIYamlFiles.con.getGameMode(player.getName(),
                    group);
            if(gameMode != null) {
    ...
    public void saveGameMode(String group) {
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            MIYamlFiles.con.saveGameMode(player.getName(), group,
                    player.getGameMode());
        } else {
            MIPlayerFile config = new MIPlayerFile(player, group);
    ...
    public void loadHunger(String group) {
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            player.setFoodLevel(MIYamlFiles.con.getHunger(player.getName(),
                    group));
            player.setSaturation(MIYamlFiles.con.getSaturation(
    ...
    public void saveHunger(String group) {
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            MIYamlFiles.con.saveHunger(player.getName(), group,
                    player.getFoodLevel());
            MIYamlFiles.con.saveSaturation(player.getName(), group,
    ...
    public void loadExperience(String group) {
        if(MIYamlFiles.config.getBoolean("useSQL")) {
            // player.setLevel(MIYamlFiles.con.getLevel(player.getName(),
            // group));
            // clear the levels
            // since the set total experience has a bug, let's just do it this
            // way...
            int[] levels = plugin.getXP(MIYamlFiles.con.getTotalExperience(
                    player.getName(), group));
            MultiInv.log.debug("Setting player level to: " + levels[0]);
            player.setLevel(levels[0]);
            player.setTotalExperience(MIYamlFiles.con.getTotalExperience(
                    player.getName(), group));
            MultiInv.log.debug("Setting player xp to: " + levels[1]);
            player.setExp((float) ((float) levels[1] / (float) levels[2]));
            // player.setTotalExperience(MIYamlFiles.con.getTotalExperience(player.getName(),
            // group));
            // player.setExp(MIYamlFiles.con.getExperience(player.getName(),
            // group));
        } else {
            MIPlayerFile config = new MIPlayerFile(player, group);
